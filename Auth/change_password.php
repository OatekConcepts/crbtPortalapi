...
// Open DB connection
$conn = getConnection();
log_action("=== CHANGE PASSWORD ATTEMPT START ===");
...
try {
    ...
    $result = $conn->query("SELECT password FROM admins WHERE id=$id AND soft_delete=0");

    if (!$result) {
        log_action("Change password query failed: " . $conn->error);
    ...
    if ($result->num_rows === 0) {
        log_action("Change password failed: id=$id not found");
    ...
    if (!password_verify($oldPassword, $admin['password'])) {
        log_action("Change password failed: old password mismatch for id=$id");
    ...
    if (!$conn->query("UPDATE admins SET password='$hashedPassword', updated_at=NOW() WHERE id=$id")) {
        log_action("Change password update failed: " . $conn->error);
        echo generateResponse(false, "An error occured", null, 500);
        closeConnection($conn);
        exit;
    }

    log_action("Password changed successfully for id=$id");

    echo generateResponse(true, "Password changed successfully.", null, 200);
} catch (\Throwable $e) {
    log_action("Change password exception: " . $e->getMessage());
    echo generateResponse(false, "An error occured", null, 500);
} finally {
    closeConnection($conn);
    log_action("=== CHANGE PASSWORD ATTEMPT END ===");
}
