...
try {
    ...
    $checkResult = $conn->query("SELECT firstname, lastname FROM admins WHERE id=$targetId AND soft_delete=0");

    if (!$checkResult) {
        log_action("Delete admin check query failed: " . $conn->error);
    ...
    if ($checkResult->num_rows === 0) {
        log_action("Delete admin failed: id=$targetId not found");
    ...
    echo generateResponse(true, "Admin deleted successfully.", null, 200);
} catch (\Throwable $e) {
    log_action("Delete admin exception: " . $e->getMessage());
    echo generateResponse(false, "An error occured", null, 500);
} finally {
    closeConnection($conn);
    log_action("=== DELETE ADMIN ATTEMPT END ===");
}
